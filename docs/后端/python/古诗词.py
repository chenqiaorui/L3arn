...
sheet = book.add_sheet('诗词', cell_overwrite_ok=True)
sheet.write(0, 0, '标题')
sheet.write(0, 1, '朝代')
sheet.write(0, 2, '作者')
sheet.write(0, 3, '内容')
...
def spider_page(url):
	...
	# 3.获取作者信息
	authors = re.findall(r'<p\sclass="source">.*?<a.*?>.*?<img.*?>(.*?)</a>', text_raw, re.DOTALL)
	...
	for value in zip(titles, dynasties, authors, contents):
		...
		sheet.write(n, 1, dynastie)
		sheet.write(n, 2, author)
		sheet.write(n, 3, content)
